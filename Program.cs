...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.OrderByDescending(s => s.EndYear).Select(s => s.EndYear).First();
        }
        ...
        static Show EndedFirstAfterTheMillennium(List<Show> shows)
        {
            ...
            return shows.OrderBy(s => s.EndYear >= 2001).First();
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Drama")).ToList();
        }
        ...
        {
            return shows.Where(s => s.EndYear - s.StartYear > 10 & s.ImdbRating < 8).OrderByDescending(s => s.Name).First();
        }
        ...
        {
            return string.Join(", ", shows.Select(s => s.Name).Take(shows.Count() - 1)) + "&" + shows.Last().Name;
        }
